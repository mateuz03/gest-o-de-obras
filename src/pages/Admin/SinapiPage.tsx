...
export default function SinapiPage() {
  ...
  // Simula o processo de envio de uma planilha
  const handleSimularUpload = () => {
    setIsUploading(true);
    setTimeout(() => {
      setIsUploading(false);
      alert("Planilha processada com sucesso! Base atualizada.");
    }, 2000);
  };
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        {/* Card Upload */}
        <Card className="border-slate-200 bg-white shadow-sm hover:border-emerald-500 transition-colors cursor-pointer group" onClick={handleSimularUpload}>
          <CardContent className="p-6 flex flex-col items-center justify-center h-full text-center">
            {isUploading ? (
              ...
              <>
                ...
                <p className="text-xs text-slate-500 max-w-[200px]">Arraste o arquivo CSV ou XLSX oficial da Caixa Econômica aqui.</p>
              </>
            )}
          </CardContent>
        </Card>
      ...
      <Card className="border-slate-200 shadow-sm bg-white overflow-hidden">
        <div className="p-6 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <div className="relative">
            ...
            <Input placeholder="Buscar arquivo..." className="pl-9 bg-slate-50 border-slate-200 h-9 w-64" />
          </div>
        </div>
            ...
            <tbody className="divide-y divide-slate-100">
              <tr className="hover:bg-slate-50/50 transition-colors">
                <td className="px-6 py-4">
                  <p className="font-bold text-slate-900">SINAPI_Custo_Ref_042026_SP.xlsx</p>
                  <p className="text-xs text-slate-400">Enviado por: Admin (Você)</p>
                </td>
                <td className="px-6 py-4 font-medium text-slate-500">15/04/2026 às 14:30</td>
                <td className="px-6 py-4 text-center font-bold text-slate-700">48.920</td>
                <td className="px-6 py-4">
                  <div className="flex flex-col">
                    <span className="font-medium text-slate-700">São Paulo (SP)</span>
                    <span className="text-xs text-slate-400">Abril 2026 - Não Desonerado</span>
                  </div>
                </td>
                <td className="px-6 py-4">
              ...
              <tr className="hover:bg-slate-50/50 transition-colors">
                <td className="px-6 py-4">
                  <p className="font-bold text-slate-900">SINAPI_Custo_Ref_032026_SP.csv</p>
                  <p className="text-xs text-slate-400">Enviado por: Sistema Automático</p>
                </td>
                <td className="px-6 py-4 font-medium text-slate-500">10/03/2026 às 02:00</td>
                <td className="px-6 py-4 text-center font-bold text-slate-700">48.815</td>
                <td className="px-6 py-4">
                  <div className="flex flex-col">
                    <span className="font-medium text-slate-700">São Paulo (SP)</span>
                    <span className="text-xs text-slate-400">Março 2026 - Não Desonerado</span>
                  </div>
                </td>
                <td className="px-6 py-4">
                  <Badge className="bg-emerald-50 text-emerald-700 border-emerald-200">Concluído</Badge>
              ...
              <tr className="hover:bg-slate-50/50 transition-colors bg-red-50/20">
                <td className="px-6 py-4">
                  <p className="font-bold text-slate-900">SINAPI_Custo_Ref_022026_SP_corrompido.xlsx</p>
                  <p className="text-xs text-slate-400">Enviado por: Admin (Você)</p>
                </td>
                <td className="px-6 py-4 font-medium text-slate-500">12/02/2026 às 11:15</td>
                <td className="px-6 py-4 text-center font-bold text-slate-700">-</td>
